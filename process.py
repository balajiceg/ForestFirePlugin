...
def run_code(dem_file,green_file,red_file,nir_file,swir_file,output_dir,NDVI,progdialog,time_step,shape_file,burnt_file):
   ...
   #reading raster
    ...
    ulti_array=np.float32(weight)*mask
...
###############################################################shape file
    ...
    for feat in fea:
        ...
        x1,y1=latlngToPix(p[0],p[1],geotransform)
        if(x1>=0 and y1>=0 and x1<ulti_array.shape[0] and y1<ulti_array.shape[1]):
            if(~np.isnan(ulti_array[x1,y1])):
                lng.append(x1)
                lat.append(y1)
...
################################################################        
    ...
    #read shape file and getdata
    no_data1=np.float32(no_data1)

    #(burni,burnj)= latlngToPix(st_lon,st_lat,geotransform)
    new = np.empty(shape=(YSize,XSize),dtype=np.float32)
    new.fill(None)

    neigh=np.empty(shape=(3,3),dtype=np.float32)
    
    for i in range(len(new)):
        for j in range(len(new[0])):
            if ulti_array[i][j]==no_data1 or ulti_array[i][j]==np.float32(0):
                new[i][j]=-1
    ...
    burnt=[]
    tmp=np.copy(new)
    tmp_ulti=np.copy(ulti_array)
    for i,j in zip(lat,lng):
        burncell(j,i,1)
        print i,j
        new[np.isnan(new)]=9999
        burnt.append(np.copy(new))
        new=np.copy(tmp)
        ulti_array=np.copy(tmp_ulti)
    ...
    try:
        ...
        iface.messageBar().pushMessage(u"Animation saving error ", str(e), level=QgsMessageBar.WARNING,15)
    plt.show()
